...
(function (global) {
  ...
  themes: {
    list: [
      {
        id: 'default',
        name: '默认黑金',
        // 【修复】原来这里是空对象占位符，切到其他皮肤再切回来时，
        // 之前皮肤设置的颜色变量不会被清空（因为"应用皮肤"是逐个写入
        // vars 里存在的变量，default 没写任何变量=什么都不覆盖，旧值
        // 会一直卡住）。这里补全成 style.css 里 :root 的原始默认值，
        // 确保切回默认皮肤时颜色能真正恢复。
        vars: {
    ...
    /* ==========================================================================
       【新增】任务详情分享卡片（见 2026-09-08 需求变更：点击任务详情卡片
       直接生成一张可保存/分享的图片，Canvas 绘制，不截图现有深色弹窗）。

       每套风格 = 一组文字颜色 + 背景（背景优先用 backgroundImage 指定的
       图片文件；不配置或图片加载失败时，回退用 gradient 数组画渐变，
       所以默认风格不需要你准备任何素材，现在就能直接用）。

       ------------------------------------------------------------------
       以后想加新风格，两种方式都行：
       1. 纯换色：照抄一份 styles 里的对象，改 gradient 颜色值即可。
       2. 用真实背景图：把图片放到
            assets/share-cards/<风格id>/bg.jpg
          然后在对应风格里把 backgroundImage 设成这个路径。图片建议
          1000×1000 正方形，暗部不要太亮，避免盖住上面的白色文字。
       ------------------------------------------------------------------
       ========================================================================== */
    shareCard: {
      // 默认使用的风格 id；目前只做了选中第一条，没有做风格选择器 UI，
      // 后续如果要给用户提供"选风格"的入口，直接在这个数组里加对象即可
      defaultStyleId: 'duolingo-orange',
      styles: [
        {
          id: 'duolingo-orange',
          name: '暖橙经典',
          backgroundImage: null, // 未提供图片，使用下面的 gradient 渐变   'assets/skins/video-fanren/1.jpg'
